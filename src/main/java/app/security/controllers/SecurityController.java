 ...
 */
//TODO: Rename to authentication controller
public class SecurityController implements IAuthenticationController {
    ...
    public Handler register() {
        return ctx -> {
            ...
            try {
                SignupRequestDTO signup = ctx.bodyAsClass(SignupRequestDTO.class);
                validateSignup(signup);                         // ensure username/password + profile field

                User created = securityDAO.createUserWithContributor(signup); // calls createUserWithContributor(dto)

                UserDTO tokenUser = new UserDTO(created.getUsername(), Set.of("USER"));
                String token = createToken(tokenUser);

                ctx.status(HttpStatus.CREATED)
                        .json(payload.put("token", token)
                                .put("username", created.getUsername()));
            } catch (ValidationException e) {
                ctx.status(HttpStatus.UNPROCESSABLE_CONTENT)
                        .json(payload.put("msg", e.getMessage()));
            } catch (EntityExistsException e) {
                ctx.status(HttpStatus.UNPROCESSABLE_CONTENT)
                        .json(payload.put("msg", "User already exists"));
            }
        };
    }
